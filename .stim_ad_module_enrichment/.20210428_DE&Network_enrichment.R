...
mk_mat = function(de, wgcna, value) {
  # given enrich list objects, generate a value matrix
  # de: diff expression genes enrich object
  # wgcna: WGCNA network genes enrich object
  # value: string of value to put in matrix
  # returns a joined matrix of both overlap matrices
  f1 = as.matrix(dcast(de$b, sample ~ group, value.var=value), 
                 rownames="sample")
  f1 = f1[de$r_names, de$c_names, drop=F]
  f2 = as.matrix(dcast(wgcna$b, sample ~ group, value.var=value), 
                 rownames="sample")
  f2 = f2[wgcna$r_names, wgcna$c_names, drop=F]
  return(rbind(f1, f2))
}

enrich = function(list1, list2, list3, gs, gs2, outname, width=7, height=7, 
                  fdr=TRUE, ...) {
  # function runs enrichment and saves to file for p and padj
  # takes two set lists, gs and gs2 background gene counts and output name and pdf width

  # DE enrich
  de_enrich = gomtable(list1, list2, gs, outname, "_DE", width, height)

  # WGCNA enrich
  wgcna_enrich = gomtable(list3, list2, gs2, outname, "_WGCNA", width, height)

  ## Make heats
  rowsplit = c(rep("DE Genes", length(de_enrich$r_names)), 
               rep("WGCNA Gene Clusters", length(wgcna_enrich$r_names)))
  # fetch Jaccard index
  f = mk_mat(de_enrich, wgcna_enrich, "Jaccard")
  # log10p heatmap
  if (isTRUE(fdr)) {
    e = mk_mat(de_enrich, wgcna_enrich, "LOGFDR")
    # filename and heatmap
    pdf(file=paste0(today, nameset, outname, "_logfdr.pdf"), width=width, 
        height=height)
    heat(e, e, f, "-log FDR", rowsplit)
    dev.off()
  } else { 
    e = mk_mat(de_enrich, wgcna_enrich, "LOGP")
    # filename and heatmap
  ...
  # OR heatmap
  c = mk_mat(de_enrich, wgcna_enrich, "odds.ratio")
  # filename and heatmap
...
heat = function(mat, pval, int, name, rowsplit=NULL) {
  ...
  # rowsplit: vector to split rows by analysis type
  ...
  a = Heatmap(mat, cluster_rows=F, cluster_columns=F, na_col="#FFFFFF", 
              ...
              row_split=rowsplit, row_gap = unit(20, "mm"), 
...
gs = length(unique(result[,Gene_id], ad[,Mmus_GeneID]))

# background WGCNA genes list
gs2 = length(unique(mod[,id], ad[,Mmus_GeneID]))

# enrich for all DE genes
enrich(de_list, ad_list, mod_list, gs, gs2, "AD_Modules", width=20, height=26)

# enrich for up/down genes
enrich(updown_list, ad_list, mod_list, gs, gs2, "AD_Modules_Up-Down", width=20, 
       height=24)
