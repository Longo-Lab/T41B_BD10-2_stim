...
##  get list of terms to plot from specified term file
# read in args
args = commandArgs(trailingOnly=TRUE)
# test if there is one argument: if not, return an error
if (length(args)==1) {
  sht_lst = fread(args[1], sep="\t", header=F)
  nameset = gsub(".txt", "", args[1])
  nameset = gsub("^\\d+_", "", nameset)
} else {
  stop("One batch or terms or none must be supplied (input file).n", call.=FALSE)
}
...
mk_heat = function(term, tbl, heat, scl="none") {
  ...
  gg = pheatmap(mat, color=rev(colors), show_rownames=T, cluster_cols=F, 
                ...
                cluster_rows=ifelse(nrow(mat) >= 2, T, F))
  return(gg[[4]])
}
...
mk_plt = function(term, tbl) {
  ...
  gg = ggplot(a, aes(x=Tiss, y=LogBH, fill=DGE)) + ylim(-lim, lim) +
    ...
    geom_text(aes(label=paste(intersection_size, "genes"), vjust=1*(sign(LogBH))), 
...
grid_pdf = function(plist, nrow=3, ncol=4, suffix="plots.pdf") {
  ...
  ggsave(paste(today, nameset, suffix, sep="_"), ml, width=420, height=594, 
         unit="mm", useDingbats=F)
}
...
# plot vst heatmap
tlist = lapply(match_terms, mk_heat, tbl=fd, heat=mean_tpm)
grid_pdf(tlist, suffix="enrich_tpm.pdf")

# plot rowscaled vst heatmap
vlist = lapply(match_terms, mk_heat, tbl=fd, heat=mean_tpm, scl="row")
grid_pdf(vlist, suffix="enrich_tpm_rs.pdf")
