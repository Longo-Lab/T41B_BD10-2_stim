...
# get file info
tiss = data.table("short"=c("1", "2", "5", "6", "9", "10"), 
                  "full"=c("Genotype Effect", "Genotype Effect w/ Stimulation",
                           "Stimulation Effect Wt", "Stimulation Effect Tg", 
                           "Drug Effect w/ Stimulation", 
...
mk_plt = function(term, tbl) {
  ...
  gg = ggplot(a, aes(x=Tiss, y=LogBH, fill=DGE)) + ylim(-lim, lim) +
    ...
    ggtitle(wrapper(term, width=30)) + scale_x_discrete(limits=tiss$full) + 
