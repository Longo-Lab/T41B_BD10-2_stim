...
f_names = c("1", "2", "5", "6", "8", "9", "10")
target_files = unlist(lapply(f_names, function(i){
  c(paste0(rootdir, paste0("/res.", i, ".csv")))
}))
# pattern = c("1", "2", "5", "6", "8", "9", "10")
replacement = c("Genotype Effect",
                "Genotype Effect w/ Stimulation", "Stimulation Effect Wt", 
                "Stimulation Effect Tg", "Drug Effect", 
                "Drug Effect w/ Stimulation", "Drug Effect Interaction")
...
enrich(de_list, ad_list, gs, "AD Modules", width=20)

# enrich for up/down genes
enrich(updown_list, ad_list, gs, "AD Modules Up-Down", width=20)
