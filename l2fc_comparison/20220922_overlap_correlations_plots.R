...
pattern = c("log2FoldChange.", "baseMean.", "\\b2", "9")
replacement = c("Log2 Fold Change ", "Base Mean Expression ", "APPL/S Effect",
                "BD10-2 Effect")
...
get_quartile_enrich = function(dt, qtl=c(1, 2, 3, 4), colx="avg_log2FC.drug", 
  ...
  if(is.list(g$result)){
    ...
    g$result$ids = lapply(g$result$intersection, get_cols, both, "Gene_id", "GeneSymbol")
    # write to file
...
lapply(c(""), function(i) {
  ...
  both = both[(pvalue.drug < 0.05 & pvalue.geno < 0.05)]
  ...
  plot_corr = function (table, x, y, title=NULL) {
    ...
    ggscatter(table, x=x, y=y, add="reg.line", conf.int=T, col="#8073ac",
      ...
      geom_text_repel(
        ...
        as.character(GeneSymbol),"")), max.overlaps=20)
  }
  p1 = plot_corr(both, "log2FoldChange.drug", "log2FoldChange.geno")
  ...
  get_quartile_enrich(both, 1, colx="log2FoldChange.drug", 
                      coly="log2FoldChange.geno", gencol="Gene_id", out=i)
  get_quartile_enrich(both, 2, colx="log2FoldChange.drug", 
                      coly="log2FoldChange.geno", gencol="Gene_id", out=i)
  get_quartile_enrich(both, 3, colx="log2FoldChange.drug", 
                      coly="log2FoldChange.geno", gencol="Gene_id", out=i)
  get_quartile_enrich(both, 4, colx="log2FoldChange.drug", 
                      coly="log2FoldChange.geno", gencol="Gene_id", out=i)
})
...
fh = lapply(qs, function(i) lapply(c(""), function(j){
  paste("20220922", nameset, j, i, "csv", sep=".")
}))
