...
rdata = '20230203_APP_Activity_Dep_dds.Rdata'
# file naming info
...
dds$group = str_replace_all(dds$Group, c("TRANS_"="", "VEH"="V", "DRUG"="D", "_STIM"="_St", "_noSTIM"="_Un"))
dds$group = factor(dds$group, levels=c("V_Un", "V_St", "D_Un", "D_St"))
...
dds$Stimulation = str_replace_all(dds$Stimulation, c("noSTIM"="Un", "STIM"="St"))
dds$Stimulation = factor(dds$Stimulation, levels=c("Un", "St"))
...
design(dds) = ~ Stimulation + Treatment + Stimulation:Treatment # interaction model
...
# result df names for lapply
res_list = c("un_drug", "st_drug", "int_drug", "stimulation")
# matched title list for plots (same order as above)
titl_list = c("Unstimulated Drug Effect", "Stimulated Drug Effect", 
              "Stimulation:Drug Interaction", "Stimulation Effect")

# extract DE genes
# the drug effect for stimulation WT(the main effect)
res.un_drug = results(dds, name="Treatment_D_vs_V", lfcThreshold=lfc)
res.un_drug.lfc = lfcShrink(dds, coef="Treatment_D_vs_V", lfcThreshold=lfc)

# the interaction term, answering: is the drug effect *different* across stimulations?
res.int_drug = results(dds, name="StimulationSt.TreatmentD")
res.int_drug.lfc = lfcShrink(dds, coef="StimulationSt.TreatmentD", lfcThreshold=lfc)

# the stimulation effect (in vehicle)
res.stimulation = results(dds, name="Stimulation_St_vs_Un")
res.stimulation.lfc = lfcShrink(dds, coef="Stimulation_St_vs_Un", lfcThreshold=lfc)

# the drug effect for stimulation Tg
# relevel stimulation for correct coef comparison to use apeglm
dds$Stimulation %<>% relevel("St","Un")
dds = nbinomWaldTest(dds)
res.st_drug = results(dds, name="Treatment_D_vs_V", lfcThreshold=lfc)
res.st_drug.lfc = lfcShrink(dds, coef="Treatment_D_vs_V", lfcThreshold=lfc)
